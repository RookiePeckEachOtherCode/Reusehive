<template>
  <t-navbar :fixed="false" :title="touserinfo.name" left-arrow style="
...
">{{ mes.content }}
        ...
        <div v-if="mes.tousername===touserinfo.name"
             style="display: flex; flex-direction: row-reverse;margin-top: 20px">
          <el-avatar :size="50" :src="userinfo.avatar" style="margin-right: 10px"
                     @click="gouserinfo(touserinfo.id)"></el-avatar>
...
">{{ mes.content }}
  ...
  <t-input v-model="text" placeholder="输入消息" style="  position: fixed;bottom: 0;width: 100%">
    <template #suffix>
      <t-button shape="round" size="medium"
                style="background-color: #70a1d7;display: flex; flex-direction: row-reverse;margin-right: 50px"
                theme="primary"
                @click="send">
        <svg class="icon" height="32" p-id="3335" style="margin-top: 10px" t="1713270149975"
